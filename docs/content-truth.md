# Portfolio Content Truth

이 파일은 모든 디자인 및 코드보다 우선한다. 수치나 문구 충돌이 발생하면 임의로 선택하지 말고 이 파일을 기준으로 한다.

이 문서는 knowledge-vault/wiki/career/profile/canonical-facts.md 의 산출물이다. 수치는 거기서만 갱신한다.

## Global

- Name: Jaehyun Han
- Positioning: Forward Deployed Platform PM / Global Commerce
- Core thesis: turning operational uncertainty into shipped systems
- Target portfolio context: Palantir Deployment Strategist

## D2D / B2B Dealer Platform

- Company: LG Electronics
- Period: 2024.07–2025.06
- Germany dealers: 1,600+
- Subsidiary order desk: fewer than 10 staff
- Order to ERP: 3.8 days
- Cancellation: 22% → 7%
- Items per order: 6.3 → 9.8
- Truck fill: 42% → 71%
- Logistics cost: −23%
- Order desk volume: 100% → 18%
- About 50% still using spreadsheets after six months: estimated
- Rollout: Germany MVP → five markets
- Award: LG Innovation Award 2025 / LG Electronics and LG Group finals 2nd

## Bunjang Partner Platform

- Company: Bunjang
- Period: 2026.01–Present
- Partners onboarded: 30
- Partners with recorded login activity in June 2026: 20 of 30
- June outbound sales represented by active partners: 88.8%
- The 88.8% measure is coverage, not attribution.
- Representative manual workflow: 5 steps → 3 steps
- Working React prototype: 16,500 lines
- Delivery squad: 12 people
- Phase 1 launched: May 2026
- Usage observation: June 2026

## ETA Seoul

- Period: 2024.01–Present
- Brands: 30
- Orders: 307
- Cumulative net sales: $41K
- H1 2026 net sales: $21.9K
- Recurring operations automated: 90%
- Catalog batch manual time: 120 → 20 minutes
- Catalog workflow manual-time reduction: −85%
- Delivery API connected in 2026
- `146K sessions` includes bot-inflated direct traffic and must not be used as a growth headline.

## SMB Segment Multi-Store

- Company: LG Electronics
- Period: 2024.03–2025.02
- Markets: 17
- SMB revenue share: 0% → 12.5% in Q4 2024
- Repurchase vs B2C: +89%
- CVR vs B2C: +2.5%p
- Refund processing: 2h → 30m
- Price errors: 8.2% → 2.4%
- Source: internal commerce KPI and market requirement records, 2024
- No illustrative intermediate ramp.
- No unverified 11-country workshop or interview count.

## Invoice Automation

- Company: LG Electronics
- Period: 2025.06–2025.08
- Scope: 30+ markets
- Manual invoice requests: 33 → 0 per month
- Manual effort removed: 165 hours per month
- Source: internal ops ticket log / implementation records, 2025
- Do not use `~21 MD` in the portfolio.
- Do not claim tax-team interviews, tax-team sign-off records, or verified-per-market approval without separate evidence.

## Forbidden values / phrases

- 286 orders
- 32 partners
- 2.8 units
- ~21 MD
- Interviews across 11 country entities
- 11-entity requirement workshops
- SMB pricing never leaked
